            ...
            with open(filepath, encoding="utf-8") as f:
                ...
                for i, entry in enumerate(entries):
                    ...
                    if match:
                        ...
                        faq_list.append((question.strip(), answer.strip()))
                    else:
...
def match_faq_answer(user_input: str, threshold: float = 0.7) -> str | None:
    ...
    user_vec = faq_vectorizer.transform([user_input])
...
# ─────────────────────────────────────────────────────────────
# SQL Template Matching (.csv → dict + similarity)
# ─────────────────────────────────────────────────────────────
sql_templates = {}
...
def load_sql_templates(path="data/sql_templates.csv"):
    ...
    try:
        ...
        if "question" in df.columns and "sql_query" in df.columns:
            ...
            print("[SQL Templates] ⚠️ Header tidak ditemukan, pakai fallback kolom 0 dan 1")
...
def match_sql_template(user_input: str, threshold: float = 0.8) -> str | None:
    ...
    best_match = difflib.get_close_matches(user_input, sql_templates.keys(), n=1, cutoff=threshold)

    if best_match:
        return sql_templates[best_match[0]]
    return None
